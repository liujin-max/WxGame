...
public class SlideScrollView : MonoBehaviour, IBeginDragHandler, IEndDragHandler
{
    ...
    void UpdateScale()
    {
        ...
        foreach (var item in items)
        {
            ...
            float offsetCoe = 1 - Mathf.Clamp(offset.magnitude, 0, 0.3f);
            item.transform.localScale = Vector3.one * offsetCoe;
            item.GetComponent<CanvasGroup>().alpha = 1 - Mathf.Clamp(offset.magnitude, 0, 0.6f);
        }
    }
...
/// </summary>
[UnityEditor.CustomEditor(typeof(SlideScrollView))]
